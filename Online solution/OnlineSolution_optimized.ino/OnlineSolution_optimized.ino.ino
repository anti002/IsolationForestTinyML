...
std::vector<float> path_length(const std::vector<vector<Node>> & forest, const std::vector<std::vector<float>> & data)
{
  ...
  Serial.print("Average single point evaluation time: ");
  Serial.println(float(totalTimeDiff)/data.size(), 17);
  return edges;
}
...
void loop() {
  ...
  for (size_t i = 0; i < NUMBER_OF_TREES; i++)
  {
    randomSeed((i + 3) * 7);
    Forest.emplace_back();
    IsolationForest(Forest[i], dataSet);
    Serial.print("Tree number  ");
    Serial.println(i + 1);
  }

  std::vector<float> anomalyScores = decision_function(Forest, dataSet);
