...
class iTree
{
    ...
    Node fit(std::vector<std::vector<float>> sub_sample)
    {
        ...
        else
        {
            ...
            //int randIndex = rand() % sub_sample.size();
            decision_node.x_value = get_split(sub_sample, decision_node.q_value); //sub_sample[randIndex][decision_node.q_value];
            
            current_node.decision_node = decision_node;
            current_node.node_id = current_node_id;
            current_node_id++;
           /* std::cout << "q: ";
            std::cout << decision_node.q_value << std::endl;
            std::cout << "x: ";
            std::cout << decision_node.x_value << std::endl;
            std::cout << "sample size: ";
            std::cout << sub_sample.size() << std::endl;*/
            ...
            /*std::cout << "Size of data right: ";
            std::cout << data_right.size() << std::endl;*/
            
            
            
           /* std::cout << "Size of data left: ";
            std::cout << data_left.size() << std::endl;*/
            ...
            node_stack.push_back(current_node);
            /*std::cout << "Depth: ";
            std::cout << this->current_height << std::endl;*/
            return current_node;
        }
    };
};
...
int main()
{
    ...
    /*std::vector<std::vector<float>> parsedCsv;
    parsedCsv.push_back({175,243,375,4455,675,765,1040});
    parsedCsv.push_back({565,347,374,364,634,8643,649,346});
    parsedCsv.push_back({6352,753,53,33,86,864,329,653});*/
...
d.push_back({    1194, 1148, 244, 610, 1698, 695});
    ...
    /*int randoms_seed = 0;
    int loops = 10;
    while (randoms_seed < loops)
    {   
        std::cout << randoms_seed << std::endl;
        srand(randoms_seed + 3*randoms_seed);
        iForest clf = iForest(NUMBER_OF_TREES, SAMPLE_SIZE);
        clf.fit(parsedCsv);
        ...
        for (size_t i = 0; i < clf.anomaly_scores.size(); i++)
        {
            ...
            averaging_scores[i] = averaging_scores[i] + clf.anomaly_scores[i];
        }
    ...
    for (size_t i = 0; i < parsedCsv.size(); i++)
    {
        averaging_scores[i] = averaging_scores[i]/loops;
        std::cout << averaging_scores[i] << std::endl;
    }*/
    for (size_t i = 0; i < clf.anomaly_scores.size(); i++)
    {
        std::cout << clf.anomaly_scores[i] << std::endl;
